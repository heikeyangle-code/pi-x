...
| 12 | 安卓运行时 | **不依赖 nodejs-mobile(仅18.x) 也不依赖 bun(无Android目标)**；主路径=自带 Termux 系 bionic **node 24 LTS**（Termux `nodejs-lts` 24.18.0-1，2026-09-05 实测）+ **bash 5.3 + 工具链**（git/rg/coreutils/termux-exec/openssl/ca-certificates，最小集见 ENGINE-BUNDLE "运行时完整性"）+ 同 UID exec（现代 Termux 0.118+ 已 targetSdk33 验证此路，本环境实测 node24+pi 在安卓运行）；兜底 B=proot（UserLAnd/Aether 路线）；兜底 C=nativeLibraryDir 放置；**绝不 targetSdk 28**；M2 真机矩阵验证 | 三方核实+本会话实测 |
