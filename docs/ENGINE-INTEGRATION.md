...
> **实现状态（已落地，单服务器，pi-only）**：`index.ts` 在同一个 `BridgeWebSocketServer` 中注入
...
> **入口已强制只走 pi**：缺 `PI_ENGINE_ENTRY` 直接拒启，codex/claude 不再作为对话引擎被拉起
> （不再需要 `PI_HOST=1` 开关）。`engineVersion` 仅作帧标签，adapter 不钉引擎版本 → 引擎更新零影响。
